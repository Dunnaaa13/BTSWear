 ...
 */
    $gallery_title       = rock_business_get_option( 'gallery_title' );
    $gallery_subtitle    = rock_business_get_option( 'gallery_subtitle' );
    $gy_content_type     = rock_business_get_option( 'gy_content_type' );
    $number_of_gy_column = rock_business_get_option( 'number_of_gy_column' );
    $number_of_gy_items  = rock_business_get_option( 'number_of_gy_items' );
    for( $i=1; $i<=$number_of_gy_items; $i++ ) :
        $gallery_page_posts[] = absint(rock_business_get_option( 'gallery_page_'.$i ) );
    endfor;
    ?>
    ...
    <?php if(!empty($gallery_title)):?>
        <div class="section-header">
            ...
            <h4 class="section-subtitle"><?php echo esc_html($gallery_subtitle);?></h4>
        </div><!-- .section-header -->
    <?php endif;?>
                    ...
                    <?php if ( has_post_thumbnail() ) : ?>
                        <div class="featured-image">
                            <a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
                                ...
                                <a class="popup" href="<?php the_permalink();?>"><i class="fa fa-search-plus"></i></a>
                            </a>
                        </div><!-- .featured-image -->
                    <?php endif; ?>
